...
def buffer_stage_sequential_initial_values_test(dut):
	...
	assert (yield dut.input_counter) == constants.ROW_STEPS-1 , 'Error in initial value of input_counter'
...
def buffer_stage_sequential_full_image_test(dut):
	...
	# generate full image of size [constants.ENCODER_VIDEO_WIDTH, constants.ENCODER_VIDEO_WIDTH]
	img = [[random.randint(0,int(pow(2,constants.VIDEO_MAX_BIT_DEPTH))-1) for _ in range(constants.NUM_IO_PIXELS)] for _ in range(constants.ROW_STEPS * constants.ENCODER_VIDEO_WIDTH)]
	...
	# feed the buffer
	for i in range(constants.ROW_STEPS * constants.ENCODER_VIDEO_WIDTH - 1):
		...
		if i > constants.ROW_STEPS-1:
			...
			assert (yield dut.output_valid) == 1, "Error in output_valid, expected 1 but 0 found in step: " + str(i)
			assert (yield dut.cached_pixels_output) == img[i - constants.ROW_STEPS], "Error in cached_pixels_output, expected array of zeros in step: " + str(i)
